...
pub fn set_asset_path(mut asset_path_res: ResMut<AssetPath>) {
    ...
    let exe_dir = std::env::current_exe().expect("error getting current exe path").parent().expect("cannot get parent dir of current exe").to_path_buf();
    
    let resources_from_exe_dir = exe_dir.parent().expect("could not get exe_dir parent").join(
        Path::new("Resources/")
    );
    ...
    for dir in check_dirs.iter() {
        ...
        for item in dir.read_dir().expect("").flatten() {
            ...
            if filename.contains("assets") {
                asset_path_res.0 = dir.as_path().to_string_lossy().to_string() + "/assets";
                return;
            }
        }
    }
